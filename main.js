...
async function getWeather(location) {
  const apiFetch = await fetch(
    `https://api.weatherapi.com/v1/forecast.json?key=fb3852c09f6447cfbb4174232242606&q=${location}&days=4`,
    { mode: "cors" }
  );
...
function updateUI(data) {
  ...
  tomorrowHigh.textContent = `High: ${Math.round(
    data.tomorrowHighTempF
  )}\u00B0`;
  ...
  toggle.addEventListener("click", () => {
    if (currentTemp.textContent === `${Math.round(data.currentTempF)}\u00B0`) {
      ...
      currentHighTemp.textContent = `H: ${Math.round(
        data.todayHighTempC
      )}\u00B0`;
      ...
      tomorrowLow.textContent = `Low: ${Math.round(
        data.tomorrowLowTempC
      )}\u00B0`;
      tomorrowHigh.textContent = `High: ${Math.round(
        data.tomorrowHighTempC
      )}\u00B0`;
      dayTwoLow.textContent = `Low: ${Math.round(data.dayTwoLowTempC)}\u00B0`;
      dayTwoHigh.textContent = `High: ${Math.round(
        data.dayTwoHighTempC
      )}\u00B0`;
